...
#pragma warning restore IDE0065
  ...
  public sealed class MultiPoseLandmarkListWithMaskAnnotation : ListAnnotation<PoseLandmarkListWithMaskAnnotation>
  {
    ...
    public void Draw(IReadOnlyList<mptcc.NormalizedLandmarks> targets, PoseLandmarkListAnnotation.BodyParts mask, bool visualizeZ = false)
    {
      if (ActivateFor(targets))
      {
        CallActionForAll(targets, (annotation, target) =>
        {
          if (annotation != null) { annotation.Draw(target, mask, visualizeZ); }
        });
      }
    }
    ...
    {
      Draw(targets, PoseLandmarkListAnnotation.BodyParts.All, visualizeZ);
    }
    ...
    protected override PoseLandmarkListWithMaskAnnotation InstantiateChild(bool isActive = true)
    {
      ...
      annotation.SetFaceCircleOptions(_drawFaceCircle, _faceCircleColor, _faceCircleWidth, _faceCirclePadding, _faceCircleRadiusScale, _faceCircleOffset);
    ...
    private void ApplyFaceCircleOptions()
    {
      foreach (var child in children)
      {
        if (child != null) { child.SetFaceCircleOptions(_drawFaceCircle, _faceCircleColor, _faceCircleWidth, _faceCirclePadding, _faceCircleRadiusScale, _faceCircleOffset); }
      }
    }
