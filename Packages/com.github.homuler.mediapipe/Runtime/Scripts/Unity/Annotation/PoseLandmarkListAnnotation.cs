...
#pragma warning restore IDE0065
  ...
  public sealed class PoseLandmarkListAnnotation : HierarchicalAnnotation
  {
    ...
    private bool _drawFaceCircle = true;
    private UnityEngine.Color _faceCircleColor = UnityEngine.Color.cyan;
    private float _faceCircleWidth = 0.35f;
    private float _faceCirclePadding = 0.0f;
    private float _faceCircleRadiusScale = 0.6f;
    private Vector2 _faceCircleOffset = Vector2.zero;
    ...
    private CircleAnnotation _faceCircleAnnotation;
    ...
    private void Start()
    {
      ...
      _faceCircleAnnotation = new GameObject("Face Circle Annotation").AddComponent<CircleAnnotation>();
      _faceCircleAnnotation.transform.SetParent(_connectionListAnnotation.transform, false);
      _faceCircleAnnotation.transform.localPosition = Vector3.zero;
      _faceCircleAnnotation.transform.localRotation = Quaternion.identity;
      _faceCircleAnnotation.transform.localScale = Vector3.one;
      _faceCircleAnnotation.SetColor(_faceCircleColor);
      _faceCircleAnnotation.SetLineWidth(_faceCircleWidth);
      _faceCircleAnnotation.SetActive(false);
    }
    ...
    public void SetFaceCircleOptions(bool drawFaceCircle, UnityEngine.Color faceCircleColor, float faceCircleWidth, float faceCirclePadding, float faceCircleRadiusScale, Vector2 faceCircleOffset)
    {
      _drawFaceCircle = drawFaceCircle;
      _faceCircleColor = faceCircleColor;
      _faceCircleWidth = faceCircleWidth;
      _faceCirclePadding = faceCirclePadding;
      _faceCircleRadiusScale = faceCircleRadiusScale;
      _faceCircleOffset = faceCircleOffset;

      if (_faceCircleAnnotation != null)
      {
        _faceCircleAnnotation.SetColor(_faceCircleColor);
        _faceCircleAnnotation.SetLineWidth(_faceCircleWidth);
        if (!_drawFaceCircle)
    ...
    private void ApplyMask(BodyParts mask)
    {
      ...
      if (!mask.HasFlag(BodyParts.Torso))
      {
        // deactivate shoulders and hips to hide the torso box
        _landmarkListAnnotation[11].SetActive(false);
        _landmarkListAnnotation[12].SetActive(false);
        _landmarkListAnnotation[23].SetActive(false);
        _landmarkListAnnotation[24].SetActive(false);
      }
    ...
    private void DrawFaceCircle()
    {
      ...
      for (var i = 0; i <= 10; i++)
      {
        var landmark = _landmarkListAnnotation[i];
        if (landmark == null || !landmark.isActiveInHierarchy)
        {
          continue;
        }

        var position = _connectionListAnnotation.transform.InverseTransformPoint(landmark.transform.position);
        min = Vector2.Min(min, new Vector2(position.x, position.y));
        max = Vector2.Max(max, new Vector2(position.x, position.y));
        zSum += position.z;
        activeCount++;
      }
      ...
      var z = zSum / activeCount;
      var center2D = (min + max) * 0.5f + _faceCircleOffset;
      var center = new Vector3(center2D.x, center2D.y, z);
      var radius = (Mathf.Max(max.x - min.x, max.y - min.y) * 0.5f + _faceCirclePadding) * _faceCircleRadiusScale;
      _faceCircleAnnotation.SetActive(true);
      _faceCircleAnnotation.Draw(center, radius);
    }
  }
}
